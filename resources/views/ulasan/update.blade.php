    ...
    <div class="main-banner wow fadeIn" id="top" data-wow-duration="1s" data-wow-delay="0.5s">
        <div class="container">
            <div class="col-lg-6 align-self-center">
                <form action="{{route('updateul',$ul->id)}}" method="POST">
                    @csrf
                    @method('put')
                    <div class="mb-3 row">
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-10">
                                <select name="pendaftaran_id" class="form-control">
                                    <option value="{{$ul->pendaftaran_id}}">{{$ul->pendaftaran->nama}}</option>
                                    @foreach ($pen as $item)
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="inputPassword" name="ulasan" value="{{$ul->ulasan}}">
                            @error('ulasan')
                                <p style="color: red">{{ $message }}</p>
                            @enderror
                        </div>
